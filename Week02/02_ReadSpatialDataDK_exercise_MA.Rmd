...
Each dataset you load is a layer! (in GIS language)
...
The *forests* and *parks* are multipolygon. *shelters* and *playgrounds* are point. 

2. What is the CRS value in these objects?
- All layers are ETRS89 / UTM zone 32 except playgrounds which is NA. 
...
<<<<<<< HEAD:Week02/02_ReadSpatialDataDK_exercise_MA.Rmd
...
Elevation: 396 rows, 876 cols (395x876)

2. What is the resolution of `aarhus` orthophoto? How many layers does it contain and what do they represent?
3334 x 3334 x 5 layers. Seems like there is an artefact layer in the data when you plot it: 
```{r}
plot(aarhus)
```
